...
final class MatchValueTest extends TestCase
{
    public function testGenerateASTValid(): void
    {
        $matcher = new MatchValue('test');
    ...
    {
        $matcher = new MatchValue('test');
        $result = $matcher->generateAST([new Token(TokenType::T_ONEOF, 'wrong')]);

        self::assertFalse($result->isValid());
        self::assertEquals('Expected token "test", got "wrong"', $result->getError());
    }
}
